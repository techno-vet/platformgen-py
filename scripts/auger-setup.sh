#!/bin/bash
# ─────────────────────────────────────────────────────────────────────────────
# Auger SRE Platform — First-Time Setup Script
#
...
#
# Usage:  bash scripts/auger-setup.sh
#         bash scripts/auger-setup.sh --venv    # force venv mode
# ─────────────────────────────────────────────────────────────────────────────
set -e
...
echo ""
echo "╔══════════════════════════════════════════════════════╗"
echo "║       Auger SRE Platform — First-Time Setup          ║"
echo "╚══════════════════════════════════════════════════════╝"
echo ""
...
[ -n "$_tok" ] && GH_TOKEN_VAL="$_tok" && GH_TOKEN_SOURCE="~/.auger/.env"
...
while [ -z "$GH_TOKEN_VAL" ]; do
    ...
    if [ -z "$user_token" ]; then
        echo "   [WARN]  Skipping — Ask Auger will not work. Add GH_TOKEN to ~/.auger/.env later."
        break
    fi
    echo -n "   Verifying... "
...
if [ -z "$LAUNCH_FLAGS" ]; then
    ...
    if [ -n "$_u" ] && [ -n "$_it" ]; then
        ART_USER="$_u"; ART_IT="$_it"; ART_SOURCE="~/.auger/.env"
    fi
...
# ── Step 4: Launch Auger ──────────────────────────────────────────────────────
echo ""
echo "[OK]  Setup complete! Launching Auger..."
echo ""

bash "$SCRIPT_DIR/auger-launch.sh" $LAUNCH_FLAGS
